...
def nats_streaming_server_env_list(ver: str) -> typing.List:
    return [
...
def nats_streaming_server_shasum_env_list(ver: str) -> typing.List:
    return [
        f"{ver}/windowsservercore-1809/Dockerfile",
    ]

def update_nats_streaming_server_env(old_ver: str, new_ver: str):
    files = nats_streaming_server_env_list(old_ver)

    r = re.compile(r"(NATS_STREAMING_SERVER )[0-9]+\.[0-9]+\.[0-9]+")
...
def update_nats_streaming_server_shasum_env(old_ver: str, new_ver: str, shasums: typing.Dict):
    files = nats_streaming_server_shasum_env_list(old_ver)

    key = f"nats-streaming-server-v{new_ver}-windows-amd64.zip"
    sha = shasums[key]

    r = re.compile(r"(NATS_STREAMING_SERVER_SHASUM )[A-Fa-f0-9]{64}")
...
def update_alpine_arch_shasums(old_ver: str, new_ver: str, shasums: typing.Dict):
    ...
    for arch in ["arm64", "arm6", "arm7", "amd64", "386"]:
        key = f"nats-streaming-server-v{new_ver}-linux-{arch}.tar.gz"
...
def get_shasums(ver: str) -> typing.Dict:
    u = f"https://github.com/nats-io/nats-streaming-server/releases/download/v{ver}/SHA256SUMS"
...
if __name__ == "__main__":
    ...
    update_nats_streaming_server_env(old_ver, new_ver)
    update_nats_tag(old_ver, new_ver)

    update_nats_streaming_server_shasum_env(old_ver, new_ver, shasums)
